    ...
    if (isNaN(parseFloat(percentage.replace(',', '.')))) {
      ...
      loading(() =>
        ...
        .then(() => {
          Alert.alert(
            'Success',
            `You will receive a notification everytime the price drops or rises by ${percentage}%`,
          );
    ...
    <View style={{backgroundColor: '#fff', flex: 1}}>
      ...
      <View style={{padding: 20}}>
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          <TouchableOpacity
            onPress={() =>
              parseFloat(percentage) - 0.5 > 0 &&
              setPercentage(p => (parseFloat(p) - 0.5).toFixed(2))
            }
            style={{
              padding: 15,
              alignItems: 'center',
          ...
          <TouchableOpacity
            onPress={() => setPercentage(p => (parseFloat(p) + 0.5).toFixed(2))}
            style={{
              padding: 15,
              alignItems: 'center',
              justifyContent: 'center',
        ...
        <View style={{flexDirection: 'row', marginTop: 20}}>
          {[0.005, 0.01, 0.015, 0.02].map(x => (
            <TouchableOpacity
              style={{flex: 1, paddingHorizontal: 5, flexDirection: 'row'}}
              key={x}
              onPress={() => setPercentage((x * 100).toFixed(2))}>
              <View
                style={{
                  flex: 1,
                  borderWidth: 1,
