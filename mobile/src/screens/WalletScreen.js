...
function Holdings(props) {
  ...
  const fullBalance = Object.entries(props.user.wallet).reduce(
    (acc, [asset, quantity]) =>
      acc +
  ...
  function handleAssetPressed(asset) {
  ...
  return (
    <FlatList
        ...
        <View
          ...
          <View style={{paddingLeft: 28, paddingTop: 28}}>
            ...
            <Text style={{color: '#fff', marginTop: 5, fontSize: 15}}>
              ...
              {(
                fullBalance /
                parseFloat(ticker24hr.map.BTCUSDT?.lastPrice ?? fullBalance)
              ).toFixed(4)}{' '}
              ₿
            </Text>
          </View>
          <View
            ...
            <Text style={{color: '#fff', marginTop: 20, fontSize: 10}}>
              This balance is not real.
            </Text>
          </View>
        </View>
      }
      refreshControl={
        <RefreshControl
          refreshing={ticker24hr.isValidating}
          onRefresh={ticker24hr.revalidate}
        />
      }
      style={{flex: 1}}
          ...
          <View style={{justifyContent: 'center', alignItems: 'flex-end'}}>
            ...
            {asset + 'USDT' in ticker24hr.map && (
              <Text style={{color: Colors.lightgray2}}>
                $
                {(
                  parseFloat(ticker24hr.map[asset + 'USDT'].lastPrice) *
                  quantity
                ).toFixed(2)}
              </Text>
            )}
