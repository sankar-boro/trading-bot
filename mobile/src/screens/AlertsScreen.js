...
export default function AlertsScreen(props) {
  ...
  function handleAlertPressed(item) {
    props.navigation.navigate('Chart', {symbol: item.data.symbol});
  }

  function handleAlertLongPressed(item) {
    Alert.alert(
      'Confirm',
      ...
      [
        {text: 'Cancel', style: 'cancel'},
        {
          ...
          style: 'destructive',
          onPress: () => {
            loading(() => authCtx.removeAlert(item.id)).catch(AlertError);
          },
        },
      ],
    );
          ...
          _ => (
            <View style={{flex: 1}}>
              <FlatList
                ...
                refreshControl={
                  <RefreshControl
                    refreshing={exInfo.isValidating}
                    onRefresh={exInfo.revalidate}
                  />
                }
                ...
                renderItem={({item}) => (
                  <TouchableOpacity
                    ...
                    <CryptoAssetImage
                      asset={exInfo.map[item.data.symbol]?.baseAsset}
                    />
                    ...
                    <View
                      ...
                      <Text style={{fontWeight: '600'}}>
                        {(item.data.percentage * 100).toFixed(2)}%
                      </Text>
                    </View>
                  </TouchableOpacity>
                )}
